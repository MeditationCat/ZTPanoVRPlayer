 ...
 */
public class VRMultiFishEyePlugin extends VRAbsPlugin {

    private VR360Program mProgram;

    private VR360Program mBitmapProgram;
    ...
    private VR360Director mFixedDirector;

    private VRDrawingCache mDrawingCache;

    public VRMultiFishEyePlugin(VRMainPluginBuilder builder, float radius, boolean isHorizontal) {
        mTexture = builder.getTexture();
        mProgram = new VR360Program(builder.getContentType());
        mBitmapProgram = new VR360Program(VRLibrary.ContentType.BITMAP);

        mProjectionModeManager = builder.getProjectionModeManager();

        mFixedDirector = new VR360DirectorFactory.OrthogonalImpl().createDirector(0);
        mConverterObject3D = new MDMesh(radius, isHorizontal);
        mDrawingCache = new VRDrawingCache();
    }
    ...
    public void init(Context context) {
        ...
        VRObject3DHelper.loadObj(context, mConverterObject3D);
    }
    ...
    private class MDMesh extends VRAbsObject3D {
        ...
        private void generateMesh(VRAbsObject3D object3D){
