...
public class SensorPacketDataObject implements Parcelable {
    ...
    // get float values
    private static final float Gyro_Sensitivity = 131.0f; // LSB/(o/s)
    private static final float PI = 3.1416f;
    public float[] getGyroValues() {
        float[] values = new float[packetDataGyro.length];
        for (int i = 0; i < packetDataGyro.length; i++) {
            values[i] = packetDataGyro[i] / Gyro_Sensitivity *(PI / 180.0f);
        }
        return values;
    }

    private static final float Acc_Sensitivity = 16384.0f; // LSB/g
    private static final float g = 9.80665f; //Earth's gravity in SI units (m/s^2)
    public float[] getAccValues() {
        float[] values = new float[packetDataAccel.length];
        for (int i = 0; i < packetDataAccel.length; i++) {
            values[i] = packetDataAccel[i] / Acc_Sensitivity * g;
        }
        return values;
    }

    private static final float Resolution_XY = 0.05f; //μT/LSB
    private static final float Resolution_Z = 0.50f; //μT/LSB

    public float[] getMagValues() {
        float[] values = new float[packetDataMSensor.length];
        values[0] = packetDataMSensor[0] * Resolution_XY;
        values[1] = packetDataMSensor[1] * Resolution_XY;
        values[2] = packetDataMSensor[2] * Resolution_Z;
        return values;
    }
}
