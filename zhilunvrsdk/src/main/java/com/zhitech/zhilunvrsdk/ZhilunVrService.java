...
public class ZhilunVrService extends Service {
    ...
    private static int mSensorState = Utils.SENSOR_STATE_NONE;
    ...
    IZhilunVrAidl.Stub mStub = new IZhilunVrAidl.Stub() {
        ...
        @Override
        public int RemoteGetSensorState() throws RemoteException {
            return mSensorState;
        }
            ...
            synchronized (this) {
                if (ACTION_USB_PERMISSION.equals(action)) {
                    ...
                    if (device != null && device.equals(usbDevice)) {
                        ...
                        mSensorState = Utils.SENSOR_STATE_CONNECTING;
                        connectToDevice();
                    }
                    ...
                    try {
                        if (device != null && device.equals(usbDevice)) {
                            ...
                            mSensorState = Utils.SENSOR_STATE_DISCONNECTED;
                            initConnection();
                            mIPacketDataCallbackList.kill();
                            mICommandResultCallbackList.kill();
                            OnCommandResultChangedCallback(0x00, null, 0);
                            //go to main activity
        ...
        try {
            ...
            if (usbConn != null) {
                if (usbInterface != null) {
                    ...
                    if (openResult) {
                        mSensorState = Utils.SENSOR_STATE_CONNECTED;
                        if (mCommandTemp != -1) {
    ...
    public void connectToDevice() {
        ...
        if (mIsRequestingPermission) {
            Utils.dLog(TAG, "device is requesting permission!");
            return;
        }
        ...
        myHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (usbConn != null && EPOUT[1] != null) {
                    ...
                    if (retVal > 0) {
                        int cmd = cmdBuffer[0] & 0xFF;
                        switch (cmd) {
                            case Utils.CMD_RECV_SENSOR_DATA:
                                mSensorState = Utils.SENSOR_STATE_RUNNING;
                                break;
                            case Utils.CMD_STOP_SENSOR_DATA:
                                mSensorState = Utils.SENSOR_STATE_STOPPED;
                                break;
                            default:
                                mSensorState = Utils.SENSOR_STATE_IDLE;
                                break;
                        }
